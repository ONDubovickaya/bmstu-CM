    ...
    while l > 2*eps
        ...
        if f1 <= f2
            ...
            if f1 <= new_f
               ...
               if debuging
                   disp('Проверим, удовлетворяет ли новая пробная точка x1 условию под *');
                   disp('>>> f1 <= new_f1, где f1 это значение ф-ции в старой пробной точке x1');
                   disp('>>> Условия под * выполнены');
                   disp(' ');
            ...
            if f2 <= new_f
                ...
                if debuging
                    disp('Проверим, удовлетворяет ли новая пробная точка x2 условию под *');
...
function [x_star, f_star] = parabols(a, b, eps, debuging, delay)
    ...
    if debuging
        fprintf('\nSTEP № %2d\n', i);
        disp('Вычислим с помощью ф-л под **');
